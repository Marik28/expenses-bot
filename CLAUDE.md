...
`ExpensesService` строит запрос, читает его через `pd.read_sql(query.statement, ...)` в DataFrame, агрегирует по категориям и рендерит графики в `BytesIO` → `InputMediaPhoto`. Текстовые таблицы отправляются как `code(...)` с `ParseMode.MARKDOWN_V2`. Категории из `settings.exclude_categories` отфильтровываются на уровне SQL.

Графики строятся на бэкенде `Agg`: `services/expenses.configure_matplotlib()` выставляет бэкенд и общие rcParams (крупный шрифт, спокойная сетка) — функция дёргается один раз в `__main__.on_startup` до старта планировщика. `_render(fig)` сохраняет фигуру с `dpi=200, bbox_inches="tight"` и закрывает её. Дневная статистика по-прежнему использует `_make_plot` (`df.plot.<type>()`, круговая + столбчатая). Статистика **за период** рисуется вручную двумя функциями, заточенными под чтение с телефона: `_category_chart` — горизонтальные столбцы по категориям с подписью суммы у каждого; `_daily_trend_chart` — расходы во времени по непрерывной оси дат (дни без трат = нули) с пунктиром среднего; периоды длиннее 92 дней агрегируются по неделям (`resample("W-MON")`). Обе функции показывают период и общую сумму в заголовке — для `/period` бот шлёт только `stats.charts`, текстовые таблицы `top_ten`/`daily` не отправляются.
